...
describe('IndexWriterConfig', () => {
  ...
  it('getRAMBufferSizeMBAsync should return number greater than zero', done => {
    new lucene.index.IndexWriterConfig(new lucene.analysis.standard.StandardAnalyzer())
      .getRAMBufferSizeMBAsync((error, value) => {
  ...
  it('getRAMBufferSizeMBPromise should return number greater than zero', async done => {
    const value = await new lucene.index.IndexWriterConfig(new lucene.analysis.standard.StandardAnalyzer()).getRAMBufferSizeMBPromise()
    expect(value).toBeGreaterThan(0)
    done()
  })
