...
describe('build and cli', () => {
  ...
  it('should print json to stdout if no --input provided', () => {
    const p = exec(`node bin/java-ast.js --jars ${javapJsonJar} --classes JavapJsonOutput,java.util.List`)
  ...
  it('should write json to file if --output provided', () => {
    ...
    const p = exec(`node bin/java-ast.js --output ${f} --jars ${javapJsonJar} --classes JavapJsonOutput,java.util.List`)
    expect(p.code).toBe(0)
    //@ts-ignore
